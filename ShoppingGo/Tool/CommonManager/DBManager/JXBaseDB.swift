...
class JXBaseDB {
    static let `default` = JXBaseDB(name: dbName)
...
//    
    ...
    func insertDatas(datas:Array<Dictionary<String,Any>>) -> Bool {
        ...
        databaseQueue?.inTransaction({ (db, rollBack) in
            ...
            datas.forEach({ (dict) in
                num += 1
                if
                    let rs = db?.executeUpdate(sql, withArgumentsIn: Array(dict.values)),
                    rs == true{
                    result = true
    ...
    /// - Returns: 更新结果
    func updateData(keyValues:Dictionary<String,Any>,condition:Array<String> = []) -> Bool {
        if isExist == false {
            return false
        }
